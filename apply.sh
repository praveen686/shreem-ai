...
original_author="praveen686"
original_name="shreem_ai"
original_urlname="shreem-ai"
original_description="Awesome shreem_ai created by praveen686" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/shreem_ai "${name}"
...
then
    ...
    echo "then run 'shreem_ai' or 'python -m shreem_ai'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'shreem_ai' or 'python -m shreem_ai' or 'poetry run shreem_ai'"
fi
